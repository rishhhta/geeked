...
# ─────────────────────────────────────────────
# 5. Загрузочный экран CLOSER (ДО запуска ComfyUI)
# ─────────────────────────────────────────────
echo "=== Устанавливаем загрузочный экран ==="

FRONTEND_DIR=$(python3 -c "import comfyui_frontend_package, os; print(os.path.dirname(comfyui_frontend_package.__file__))" 2>/dev/null || echo "")

if [[ -n "$FRONTEND_DIR" ]]; then
    STATIC_DIR="${FRONTEND_DIR}/static"

    wget -q -O "${STATIC_DIR}/CLOSER.png" \
        "https://raw.githubusercontent.com/rishhhta/HeavenMode/main/CLOSER.png"

    INDEX="${STATIC_DIR}/index.html"
    if [[ -f "$INDEX" ]]; then
        SPLASH='<style id="closer-splash">body{background:#000 url("/CLOSER.png") center center no-repeat !important;background-size:contain !important;}</style>'
        sed -i "s|</head>|${SPLASH}</head>|" "$INDEX"
        echo "Загрузочный экран установлен"
    fi
else
    echo "Frontend dir not found, пропускаем splash"
fi
...
# ─────────────────────────────────────────────
# 8. Воркфлоу по умолчанию
# ─────────────────────────────────────────────
echo "=== Загружаем воркфлоу ==="
mkdir -p "${COMFYUI_DIR}/user/default/workflows"

wget -q -O "${COMFYUI_DIR}/user/default/workflows/dancim2.0.json" \
    "https://raw.githubusercontent.com/rishhhta/HeavenMode/main/dancim2.0.json"

wget -q -O "${COMFYUI_DIR}/user/default/workflows/closer_lipsync.json" \
    "https://raw.githubusercontent.com/rishhhta/HeavenMode/main/closer%20lipsync%20wf.json"

echo "Воркфлоу установлены"
